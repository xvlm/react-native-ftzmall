...
export function fetchArticles(isRefreshing, loading, typeId, isLoadMore, page = 1) {
  return dispatch => {
    ...
    return request(`${urls.ad_brand}${typeId}`, 'get')
...
export function fetchImageText(isRefreshing, loading, typeId, isLoadMore, page = 1) {
  return dispatch => {
    dispatch(fetchArticleList(isRefreshing, loading, isLoadMore));
    // return request(`${WEXIN_ARTICLE_LIST}?typeId=${typeId}&page=${page}`, 'get')
...
export function fetchBrand(isRefreshing, loading, typeId, isLoadMore, page = 1) {
  return dispatch => {
    dispatch(fetchArticleList(isRefreshing, loading, isLoadMore));
    // return request(`${WEXIN_ARTICLE_LIST}?typeId=${typeId}&page=${page}`, 'get')
    return request(`${urls.ad_brand}${typeId}`, 'get')
...
export function fetchProduct(isRefreshing, loading, typeId, isLoadMore, page = 1) {
  return dispatch => {
    dispatch(fetchArticleList(isRefreshing, loading, isLoadMore));
    // return request(`${WEXIN_ARTICLE_LIST}?typeId=${typeId}&page=${page}`, 'get')
